...
def get_oci_config_from_options(options):
    ...
    config_file = os.path.expanduser(options.get("config_file_location", "~/.oci/config"))
...
def create_service_client_from_options(options, client_class, region=None):
    """Create an OCI client from inventory plugin options."""
    ...
    if auth_type == "instance_principal":
        ...
        if not token_file:
            raise ValueError("Session token auth requires security_token_file in the selected OCI config profile.")
