...
short_description: Manage a block volume group backup resource in Oracle Cloud Infrastructure
description:
  - Create, update, and delete OCI block volume group backups.
  - A volume group backup is a point-in-time copy of a volume group's member volumes that can be
    used to restore the volume or create new volumes.
  - Use M(oracle.oci.oci_volume_group_backup_info) to list or fetch volume group backups.
  - Uses the shared OCI helper layer for authentication, waiting, retry
    behavior, and result shaping.
  - Create requests must omit C(volume_group_backup_id). After create, capture the
...
options:
  ...
  compartment_id:
    description:
      - The OCID of the compartment used to scope name-based backup lookups
        when C(volume_group_backup_id) is omitted.
  ...
  volume_group_id:
    description:
      ...
      - Required when creating a backup.
      - Also scopes name-based backup lookups when C(volume_group_backup_id) is
        omitted.
...
class OciVolumeGroupBackupModule(OciResourceBase):
    """Concrete resource adapter for OCI block volume group backups."""
    ...
    enum_keys = ENUM_KEYS
    # volume_group_id, type, and kms_key_id are create-only. They either have no
