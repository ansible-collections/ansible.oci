...
DOCUMENTATION = r"""
---
module: oci_network_service_gateway_info
short_description: Retrieve Service Gateway information from Oracle Cloud Infrastructure
description:
  - Retrieve details about one or more OCI service gateways.
  - Use C(service_gateway_id) to fetch a single service gateway, or
    C(compartment_id) to list service gateways in a compartment.
...
options:
  compartment_id:
    description:
      - The OCID of the compartment to list service gateways from.
  ...
  vcn_id:
    description:
      - Filter listed service gateways by VCN.
...
- name: List all service gateways in a compartment
  ansible.oci.oci_network_service_gateway_info:
    compartment_id: ocid1.compartment.oc1..example

- name: List service gateways in a VCN by name
  ansible.oci.oci_network_service_gateway_info:
    compartment_id: ocid1.compartment.oc1..example
    vcn_id: ocid1.vcn.oc1..example
    name: example-service-gateway

- name: Get a specific service gateway
  ansible.oci.oci_network_service_gateway_info:
...
RETURN = r"""
service_gateways:
  description: List of service gateways that matched the query.
  returned: always
  type: list
  elements: dict
  contains:
...
class OciNetworkServiceGatewayInfoModule(OciInfoBase):
    """Concrete info adapter for OCI service gateways."""
    ...
    resource_get_method = "get_service_gateway"
...
def main():
    ...
    OciNetworkServiceGatewayInfoModule(module).execute_info_module()
