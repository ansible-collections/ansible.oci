...
# A volume attachment uses its own lifecycle vocabulary
...
class OciVolumeAttachmentModule(OciResourceBase):
    ...
    issuing an update. The attachment becomes terminal via ``DETACHED`` rather
    than the collection-wide ``DEAD_STATES`` vocabulary, so present/absent
    idempotency is normalized here the same way ``oci_drg_attachment`` does.
    """
    ...
    def resolve_target_resource(self):
        ...
        A volume attachment becomes terminal via ``DETACHED`` instead, so this
    ...
    def delete_resource(self, resource):
        try:
            ...
            if getattr(exc, "status", None) == 409:
                self.module.fail_json(
                    msg=(
                        f"Cannot detach {self.create_resource_name} {resource.id} while "
        ...
        return self._wait_for_volume_attachment_detached(resource.id)

    def _wait_for_volume_attachment_detached(self, volume_attachment_id):
        """Wait for a volume attachment to reach DETACHED or disappear.
        ...
        (``DELETED``/``TERMINATED``). A volume attachment instead becomes
        terminal via ``DETACHED``, so this mirrors the shared wait helper with
        the correct target state and treats a 404 as already detached at any
        point in the wait.
        """
        ...
        try:
            initial_response = self.get_resource_response(volume_attachment_id)
        except Exception as exc:
