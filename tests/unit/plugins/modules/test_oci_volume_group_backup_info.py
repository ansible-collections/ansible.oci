...
def test_main_requires_compartment_id_or_backup_id(monkeypatch):
    ...
    monkeypatch.setattr(module_obj, "OciVolumeGroupBackupInfoModule", FakeInfoModule)

    module_obj.main()

    assert captured["run_called"] is True
    assert captured["required_one_of"] == [["compartment_id", "volume_group_backup_id"]]
...
def test_fetch_resources_prefers_id_lookup(monkeypatch):
    ...
    def get_volume_group_backup(**kwargs):
        ...
        return FakeResponse(
            data=FakeModel(
                id=kwargs["volume_group_backup_id"], display_name="example-backup"
            )
        )
    ...
    instance = make_volume_group_backup_info_module(
        ...
        client=types.SimpleNamespace(get_volume_group_backup=get_volume_group_backup),
    )
    monkeypatch.setattr(
        instance,
        "list_all_resources",
    ...
    assert get_calls == [{"volume_group_backup_id": "ocid1.volumebackup.oc1..example"}]
