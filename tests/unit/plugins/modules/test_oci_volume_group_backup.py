...
VOLUME_GROUP_BACKUP_MODEL_NAMES = (
...
def install_fake_oci(monkeypatch):
    return shared_install_fake_oci(
        monkeypatch,
        model_names=VOLUME_GROUP_BACKUP_MODEL_NAMES,
    )


def make_volume_group_backup_module(module_obj, params, client=None):
    return make_module_instance(
        module_obj,
        "OciVolumeGroupBackupModule",
        params,
        client=client,
    )
...
def test_main_exposes_expected_arguments(monkeypatch):
    ...
    module_obj = load_collection_module("oci_volume_group_backup")
    captured = {}
    ...
    class FakeVolumeGroupBackupModule:
    ...
    monkeypatch.setattr(
        module_obj, "OciVolumeGroupBackupModule", FakeVolumeGroupBackupModule
    )

    module_obj.main()

    assert captured["run_called"] is True
    assert captured["argument_spec"]["volume_group_backup_id"] == {"type": "str"}
...
def test_build_create_volume_group_backup_details_includes_supported_fields(monkeypatch):
    install_fake_oci(monkeypatch)

    backup_module = load_collection_module("oci_volume_group_backup")
    details = backup_module.build_create_volume_group_backup_details(
        {
    ...
    assert details.volume_group_id == "ocid1.volumegroup.oc1..example"
    assert details.compartment_id == "ocid1.compartment.oc1..example"
    assert details.display_name == "example-group-backup"
...
def test_build_create_volume_group_backup_details_omits_unset_optional_fields(monkeypatch):
    install_fake_oci(monkeypatch)
...
def test_build_update_plan_maps_backup_fields_to_update_model(monkeypatch):
    ...
    backup_module = load_collection_module("oci_volume_group_backup")
    instance = make_volume_group_backup_module(
        backup_module,
        {"name": "updated-group-backup"},
    )
    resource = FakeModel(
        id="ocid1.volumegroupbackup.oc1..example",
        display_name="current-group-backup",
    )

    update_plan = instance.build_update_plan(resource)

    assert update_plan["update_needed"] is True
    assert update_plan["update_model_fields"] == {"display_name": "updated-group-backup"}
...
def test_build_update_details_wraps_retention_period(monkeypatch):
    ...
    backup_module = load_collection_module("oci_volume_group_backup")
    instance = make_volume_group_backup_module(backup_module, {})

    details = instance.build_update_details(
        {
            "display_name": "updated-group-backup",
            "retention_period": {
    ...
    assert details.display_name == "updated-group-backup"
    assert isinstance(details.retention_period, FakeModel)
...
def test_needs_update_ignores_create_only_type(monkeypatch):
    ...
    instance = make_volume_group_backup_module(
        backup_module,
        {"name": "current-group-backup", "type": "full"},
    )
    resource = FakeModel(
        id="ocid1.volumegroupbackup.oc1..example",
        display_name="current-group-backup",
...
def test_needs_update_rejects_volume_group_id_drift(monkeypatch):
    install_fake_oci(monkeypatch)

    backup_module = load_collection_module("oci_volume_group_backup")
    instance = make_volume_group_backup_module(
        backup_module,
        {"volume_group_id": "ocid1.volumegroup.oc1..desired"},
    )
    resource = FakeModel(
        id="ocid1.volumegroupbackup.oc1..example",
        volume_group_id="ocid1.volumegroup.oc1..current",
    )

    with pytest.raises(FailJsonCalled) as exc_info:
        instance.needs_update(resource)

    assert "volume_group_id" in exc_info.value.payload["msg"]
...
def test_create_resource_uses_create_volume_group_backup_and_waits(monkeypatch):
    install_fake_oci(monkeypatch)

    backup_module = load_collection_module("oci_volume_group_backup")
    create_calls = []
    response = FakeResponse(data=FakeModel(id="ocid1.volumegroupbackup.oc1..example"))

    def create_volume_group_backup(create_volume_group_backup_details):
        create_calls.append(create_volume_group_backup_details)
        return response
    ...
    instance = make_volume_group_backup_module(
        ...
        client=types.SimpleNamespace(
            create_volume_group_backup=create_volume_group_backup
        ),
    )
    monkeypatch.setattr(
        instance,
    ...
    assert create_calls[0].volume_group_id == "ocid1.volumegroup.oc1..example"
    assert create_calls[0].compartment_id == "ocid1.compartment.oc1..example"
    assert create_calls[0].display_name == "example-group-backup"
    assert create_calls[0].type == "INCREMENTAL"
    assert resource.id == "ocid1.volumegroupbackup.oc1..example"
    assert resource.lifecycle_state == "AVAILABLE"


def test_update_resource_uses_update_volume_group_backup_details_and_waits(monkeypatch):
    install_fake_oci(monkeypatch)
    ...
    def update_volume_group_backup(
        volume_group_backup_id, update_volume_group_backup_details
    ):
        update_calls.append(
            (volume_group_backup_id, update_volume_group_backup_details)
        )
    ...
    resource = FakeModel(id="ocid1.volumegroupbackup.oc1..example")
    instance = make_volume_group_backup_module(
        backup_module,
        {
            "name": "updated-group-backup",
        ...
        client=types.SimpleNamespace(
            update_volume_group_backup=update_volume_group_backup
        ),
    )
    monkeypatch.setattr(
        instance,
    ...
    assert update_calls[0][0] == "ocid1.volumegroupbackup.oc1..example"
    assert update_calls[0][1].display_name == "updated-group-backup"
    assert updated_resource.id == "ocid1.volumegroupbackup.oc1..example"


def test_delete_resource_uses_delete_volume_group_backup_and_waits(monkeypatch):
    install_fake_oci(monkeypatch)
    ...
    def delete_volume_group_backup(volume_group_backup_id):
        delete_calls.append(volume_group_backup_id)
        return response
    ...
    instance = make_volume_group_backup_module(
        ...
        {"wait": True},
        client=types.SimpleNamespace(
            delete_volume_group_backup=delete_volume_group_backup
        ),
    )
    monkeypatch.setattr(
        instance,
    ...
    assert delete_calls == ["ocid1.volumegroupbackup.oc1..example"]
...
def test_create_required_fields_enforced(monkeypatch):
    ...
    assert "Creating a volume group backup requires" in exc_info.value.payload["msg"]
    assert "volume_group_id" in exc_info.value.payload["msg"]
...
def test_name_lookup_requires_compartment_id(monkeypatch):
    ...
    assert (
        "Using name lookup for volume group backup requires"
        in exc_info.value.payload["msg"]
    )
    assert "compartment_id" in exc_info.value.payload["msg"]


def test_name_lookup_requires_volume_group_id(monkeypatch):
    install_fake_oci(monkeypatch)
