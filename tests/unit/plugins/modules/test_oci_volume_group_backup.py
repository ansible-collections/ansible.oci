...
def test_main_exposes_expected_arguments(monkeypatch):
    ...
    monkeypatch.setattr(module_obj, "OciVolumeGroupBackupModule", FakeVolumeGroupBackupModule)
...
def test_build_create_volume_group_backup_details_includes_supported_fields(monkeypatch):
    ...
    assert details.volume_group_id == "ocid1.volume.oc1..example"
...
def test_create_resource_uses_create_volume_group_backup_and_waits(monkeypatch):
    ...
    instance = make_volume_group_backup_module(
        ...
        client=types.SimpleNamespace(create_volume_group_backup=create_volume_group_backup),
    )
    monkeypatch.setattr(
        instance,
        "call_with_retry",
    ...
    assert create_calls[0].volume_group_id == "ocid1.volume.oc1..example"
...
def test_update_resource_uses_update_volume_group_backup_details_and_waits(monkeypatch):
    ...
    def update_volume_group_backup(volume_group_backup_id, update_volume_group_backup_details):
        update_calls.append((volume_group_backup_id, update_volume_group_backup_details))
        return response
    ...
    instance = make_volume_group_backup_module(
        ...
        client=types.SimpleNamespace(update_volume_group_backup=update_volume_group_backup),
    )
    monkeypatch.setattr(
        instance,
        "call_with_retry",
...
def test_delete_resource_uses_delete_volume_group_backup_and_waits(monkeypatch):
    ...
    instance = make_volume_group_backup_module(
        backup_module,
        {"wait": True},
        client=types.SimpleNamespace(delete_volume_group_backup=delete_volume_group_backup),
    )
    monkeypatch.setattr(
        instance,
        "call_with_retry",
...
def test_name_lookup_requires_compartment_id(monkeypatch):
    ...
    assert "Using name lookup for volume group backup requires" in exc_info.value.payload["msg"]
