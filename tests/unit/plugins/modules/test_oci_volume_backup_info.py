...
def make_volume_backup_info_module(module_obj, params, client=None):
    return make_module_instance(
        module_obj,
        "OciVolumeBackupInfoModule",
        params,
        client=client,
    )
...
def test_main_requires_compartment_id_or_backup_id(monkeypatch):
    ...
    module_obj = load_collection_module("oci_volume_backup_info")
    captured = {}
    ...
    monkeypatch.setattr(module_obj, "OciVolumeBackupInfoModule", FakeInfoModule)

    module_obj.main()

    assert captured["run_called"] is True
    assert captured["required_one_of"] == [["compartment_id", "volume_backup_id"]]


def test_fetch_resources_prefers_id_lookup(monkeypatch):
    install_fake_oci(monkeypatch)

    info_module = load_collection_module("oci_volume_backup_info")
    get_calls = []

    def get_volume_backup(**kwargs):
        get_calls.append(kwargs)
        return FakeResponse(
            data=FakeModel(
                id=kwargs["volume_backup_id"], display_name="example-backup"
            )
        )

    instance = make_volume_backup_info_module(
        info_module,
        {"volume_backup_id": "ocid1.volumebackup.oc1..example"},
        client=types.SimpleNamespace(get_volume_backup=get_volume_backup),
    )
    monkeypatch.setattr(
        instance,
        "list_all_resources",
    ...
    assert resources[0].id == "ocid1.volumebackup.oc1..example"
    assert get_calls == [{"volume_backup_id": "ocid1.volumebackup.oc1..example"}]
...
def test_fetch_resources_lists_by_supported_filters(monkeypatch):
    ...
    instance = make_volume_backup_info_module(
        ...
        client=types.SimpleNamespace(list_volume_backups="list_method"),
    )
    monkeypatch.setattr(
        instance,
        "list_all_resources",
...
def test_run_returns_volume_backups_key(monkeypatch):
    install_fake_oci(monkeypatch)
